...
const RESPONSE_SCHEMA = {
  ...
  properties: {
    ...
    company_code: { type: 'STRING', nullable: true, description: '左上1段目の2桁のコード番号' },
    company_name: { type: 'STRING', nullable: true },
    division_code: { type: 'STRING', nullable: true, description: '左上2段目の3桁のコード番号' },
    division_name: { type: 'STRING', nullable: true },
    branch_code: { type: 'STRING', nullable: true, description: '左上3段目の2桁のコード番号' },
    branch_name: { type: 'STRING', nullable: true },
    store_code: { type: 'STRING', nullable: true, description: '左上4段目の2桁のコード番号' },
    ...
    approval_no_main: {
      type: 'STRING',
      nullable: true,
      description: '承認NO欄のメインの番号(桁数の多い方の枠に印字されている数字)。枠の桁数どおりに先頭の0も含めてそのまま入れる',
    },
    approval_no_suffix: {
      type: 'STRING',
      nullable: true,
      description: '承認NO欄のメイン番号の右隣にある、1桁だけの別枠の数字。存在しない場合はnull',
    },
    inspection_date: { type: 'STRING', nullable: true, description: '検収月日。手書きの場合も読み取ってYYYY-MM-DD形式に変換' },
    remittance_form: { type: 'STRING', nullable: true, description: '振込専用用紙欄の数字' },
...
- 「承認NO」欄は、桁数の多いメインの枠と、その右隣にある1桁だけの別枠の、2つの枠に分かれていることがあります。この2つは必ず別々の値(approval_no_main / approval_no_suffix)として抽出し、連結しないでください。桁を省略したり0を落としたりせず、枠の桁数どおりに読み取ってください。
- 左上には4段のコード番号欄(2桁・3桁・2桁・2桁)とそれぞれに対応する名称が並んでいます。コード番号と名称をそれぞれ別の項目として抽出してください。
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 承認NOはメイン番号と1桁の補助番号が別枠になっていることがあるため、
    // モデルの出力揺れ(補助番号を含めたり含めなかったり)を吸収し常に同じ形式で連結する。
    const approvalNoMain = (extracted.approval_no_main || '').trim() || null;
    const approvalNoSuffix = (extracted.approval_no_suffix || '').trim() || null;
    const approvalNo = approvalNoMain
      ? approvalNoSuffix
        ? `${approvalNoMain}-${approvalNoSuffix}`
        : approvalNoMain
      : null;
    ...
    const { data: inserted, error: insertError } = await supabase
      ...
      .insert({
        ...
        company_code: extracted.company_code || null,
        company_name: extracted.company_name || null,
        division_code: extracted.division_code || null,
        division_name: extracted.division_name || null,
        branch_code: extracted.branch_code || null,
        branch_name: extracted.branch_name || null,
        store_code: extracted.store_code || null,
        ...
        remittance_form: extracted.remittance_form || null,
